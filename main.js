...
// ==========================================
const CONFIG = {
  goldCount: 2000, // Hạt vàng
  redCount: 300, // Số lượng đèn đỏ
  giftCount: 150, // Số lượng hộp quà
  explodeRadius: 65,
  photoOrbitRadius: 25,
  treeHeight: 120,
  treeBaseRadius: 60,
...
function init3D() {
  ...
  // Xóa fog để background hiển thị rõ hơn
  ...
  // Vandiep
  groupGold = createParticleSystem("gold", CONFIG.goldCount, 4.0);
  groupRed = createParticleSystem("red", CONFIG.redCount, 7.0); // Vandiep
  groupGift = createParticleSystem("gift", CONFIG.giftCount, 6.0); // Vandiep
...
function createDecorations() {
  ...
  canvas.height = 256;
  const ctx = canvas.getContext("2d");
  ctx.font = 'bold italic 140px "Times New Roman"';
  ctx.fillStyle = "#FFD700";
  ctx.textAlign = "center";
  ctx.shadowColor = "#FF0000";
  ctx.shadowBlur = 60;
  ctx.fillText("MERRY CHRISTMAS", 768, 160);
  ...
  titleMesh = new THREE.Mesh(new THREE.PlaneGeometry(120, 20), mat);
  titleMesh.position.set(0, 85, 0);
  scene.add(titleMesh);
...
// ==========================================
// 4. Vandiep - Camera và Hand Tracking
// ==========================================
function startCamera() {
  ...
  if (!video || !canvas) {
    console.log("Video hoặc canvas không tồn tại");
    return;
  }
  ...
  hands.onResults((results) => {
    ...
    ctx.drawImage(results.image, 0, 0, 240, 180);
